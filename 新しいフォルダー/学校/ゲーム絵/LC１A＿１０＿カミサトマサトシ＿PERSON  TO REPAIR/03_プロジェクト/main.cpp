...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nCmdShow)
{
	...
	//プレイヤー情報
	int player_posx = 300;				//ｘ座標
	int player_posy = 800;				//ｙ座標
	int player_speed = 8;				//プレイヤーの速さ
	...
	while (1)
	{
		...
		if (player_posx <= 600 - 35)		//ここから下は操作設定↓
		{
		...
		switch (scene)
		{
			...
			case 0:
				...
				boss_main_posy = 300;				//ボスメイン機Y
				boss_main_hp = 120;				//HP
				boss_sub1_posx = 200;				//ボスサブ１X
				boss_sub1_posy = 100;				//ボスサブ１Y
				boss_sub1_hp = 30;				//HP
				boss_sub2_posx = 400;				//ボスサブ２X
				boss_sub2_posy = 100;				//ボスサブ２Y
				boss_sub2_hp = 30;				//HP
			...
			case 1://シーン切り替えようバトル画面。
				...
				if (keys[KEY_INPUT_S] == 1 && prev[KEY_INPUT_S] == 0)
				{
					scene = 2;
				}
				if (player_flag == 1)
				...
				//ボス戦突入
				if (time > 5400 && boss_main_flag == 0 && boss_sub1_flag == 0 && boss_sub2_flag == 0)
				{
					boss_main_flag = 1;
					boss_sub1_flag = 1;
					boss_sub2_flag = 1;
				...
				{
					boss_speed0_posx = GetRand(15 + 1);
					boss_speed0_posy = GetRand(15 + 1);
					boss_speed0_change = 0;
				}
				if (boss_speed1_change >= 3)
				{
					boss_speed1_posx = GetRand(10 + 1);
					boss_speed1_posy = GetRand(10 + 1);
					boss_speed1_change = 0;
				}
				if (boss_speed2_change >= 3)
				{
					boss_speed2_posx = GetRand(10 + 1);
					boss_speed2_posy = GetRand(10 + 1);
				...
				for (int i = 0; i < enemy_number; i++)
				{
					if (enemy_flag[i] == 0 && boss_main_flag == 0 && stop_flag == 0)
					{
						boom_time[i] -= 1;
						enemy_respown[i] -= 1;
				...
				DrawFormatString(17, 400, GetColor(83, 160, 162), "bdg [%d]", bdg);//スコア
				//DrawFormatString(17, 50, GetColor(83, 160, 162), "現在のスコア [%d]", score);//スコア
				//DrawFormatString(17, 75, GetColor(83, 160, 162), "次のレベルアップまで [%d]", shot_upgrade - score);
