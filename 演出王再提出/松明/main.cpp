...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nCmdShow)
{
	...
	const int AMOUNT = 10;
	double efectPosX[AMOUNT] = { 0 };
	double efectPosY[AMOUNT] = { 0 };
	int efectSpace = 32;//もう片方に出すときの間感
	float efectSize[AMOUNT] = { 0 };
	float efectSpeed = 0.5f;
	int efectHp[AMOUNT] = { 0 };//ある意味フラグ。
	int landingTime = 20;
	int left = 0;
	int right = 0;
	...
	while (1)
	{
		...
		for (int i = 0; i < 5; i++)
		{
			if (efectHp[i] <= 0 && keys[KEY_INPUT_SPACE])
			{
				...
				efectPosX[i] = playerPosX + GetRand(10) - 10;
		...
		for (int i = 0; i < 5; i++)
		{
			if (efectHp[i] >= 1)
			{
				...
				DrawCircle(efectPosX[i], efectPosY[i], efectSize[i], GetColor(255, 0, 0), TRUE);
